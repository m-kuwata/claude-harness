...
**plan（プランナー）→ plan-review（計画レビュー）の entry ゲート2本はデフォルトで
組み込む。** `docs/harness.example.yaml` の標準構成であり、外すかどうかだけを
確認すれば足りる（聞き方の例: 「実装着手前に独立コンテキストのプランナーへ
計画を立てさせ、さらに別の独立ペルソナがその計画をレビューして承認するまで
実装をブロックする entry ゲートをデフォルトで入れます。不要ならその旨教えて
ください」）。plan-review は `reads: plan` で計画を読み、却下されると
プランナー・レビュアーどちらもコードを編集する権限を持たないため、
**ユーザーまたは実装担当の Claude が計画を修正して再レビューを依頼する**運用になる
（機械的な自動再試行ループではない）ことを説明しておくとよい。
プランナー・レビュアーには `docs/architecture.md` 等の設計資料を `context:` として
渡すのが望ましいので、
ステップ1の調査で見つけた資料を割り当てる。
...
`.gitignore` に以下を追記（レビュー系ゲート等が書き出すセッションごとの
一時アーティファクトはコミット対象外にする。ただし `.claude-harness/progress.log`
自体はセッションをまたぐ永続的な進捗記録なので**追跡対象のまま**にする）:
