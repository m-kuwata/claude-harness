...
**plan（プランナー）entry ゲートはデフォルトで組み込む。** `docs/harness.example.yaml`
の標準構成であり、外すかどうかだけを確認すれば足りる（聞き方の例:
「実装着手前に独立コンテキストのプランナーへ計画を立てさせる entry ゲートを
デフォルトで入れます。不要ならその旨教えてください」）。プランナーには
`docs/architecture.md` 等の設計資料を `context:` として渡すのが望ましいので、
