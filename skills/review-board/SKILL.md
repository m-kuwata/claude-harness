...
`.claude-harness/review-findings.json` に書き出す：
...
- `.claude-harness/` ディレクトリがなければ作成する
...
- skill: review-board
  when: impl
  personas: [qa, po]
  verify: "bash ${HARNESS_PLUGIN_ROOT}/scripts/verify-approved.sh .claude-harness/review-findings.json"
```
