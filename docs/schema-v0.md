...
| 15 | 計画修正の実行者 | plan-reviewer が却下しても、planner/plan-reviewer どちらも Write/Edit を持たないため計画ファイルを直接書き換えられない。実際に書き換えるのは `/gate-run` を実行しているメインコンテキストの Claude（プランナーを再起動して改訂させるか、自身で編集する）。完全に自動化されたリトライループではなく、メインコンテキストの判断が挟まる設計 |
