...
| 8 | プラグイン名・マーケットプレイス配置 | **v0.9.0 で解決**。プラグイン名 `claude-harness`、マーケットプレイスはこのリポジトリ自身（`.claude-plugin/marketplace.json`、マーケットプレイス名 `m-kuwata`）を転用する方式で確定。`claude plugin marketplace add <このリポジトリのパス/URL>` → `claude plugin install claude-harness@m-kuwata` の実インストールで動作確認済み |
...
| 10 | `${CLAUDE_PLUGIN_ROOT}` が eval される任意コマンド内で実環境変数として使えるか | **部分的に v0.9.0 で解決**。`hooks.json` の `command` フィールド内（`bash ${CLAUDE_PLUGIN_ROOT}/hooks/session-start.sh` 等）では実地インストールで正しく展開されることを確認した（SessionStart/PreToolUse とも実際に発火）。ただし harness.yaml の `verify:`/`run:`/`inventory:`（エンジンが独自に `eval` する文字列）内での展開は未検証のまま — 検証の必要自体がなくなった。エンジン自身が `HARNESS_PLUGIN_ROOT` を export する方式で確定し、`${CLAUDE_PLUGIN_ROOT}` への依存に切り替える計画はなし |
...
| 13 | Planner が立てた計画のレビュー（Sprint contracts の完全形） | **v0.7.0 で解決**。`entry.gates` に `plan` → `plan-review`（`reads: plan`・別ペルソナ `plan-reviewer`）をデフォルト搭載。却下（`approved:false`）は `verify:` 失敗として同じトークンで再ブロックされ、既存のノンス機構のまま「計画修正→再レビュー」ループが成立する。新しいエンジン機構は `gates[].reads`（他ゲートの成果物参照）のみ追加した |
| 14 | Planner の計画とその後の実装の追従確認 | 未解決。plan-review はあくまで「着手前の計画」を承認するだけで、実装がその計画通りに進んでいるかを機械チェックする仕組みはない（review-board 等の後続ゲートが実質的な代替になっているが、計画の「スコープ外」逸脱を実装後に直接検出する仕組みではない） |
| 15 | 計画修正の実行者 | plan-reviewer が却下しても、planner/plan-reviewer どちらも Write/Edit を持たないため計画ファイルを直接書き換えられない。実際に書き換えるのは `/gate-run` を実行しているメインコンテキストの Claude。**v0.8.0 で「却下フィードバックの引き継ぎ」を自動化**（`gate-run-prep.sh` が reads の逆引きで却下している reviewer を検出し `revision_from`/`revision_feedback` を自動的にバンドルへ含める）。ただし「プランナーを再起動するかどうか・いつ再起動するか」の判断自体はメインコンテキストが行う。完全自動リトループではなく、メインコンテキストが一枚噛む設計のまま |
| 16 | `personas.<name>.agent` の短縮形（`harness:X`）は実際の `subagent_type` と一致するか | **v0.9.0 で解決（実地インストールで発覚したバグ）**。Claude Code がプラグイン install 時に登録する `subagent_type` は `<plugin.json の name>:<名前>` であり、決め打ちの `harness:` 短縮形とは無関係だった（このプラグインでは実際には `claude-harness:X`）。v0.8.0 までの全検証は `Agent` ツールへ `general-purpose` を使いペルソナ定義をプロンプトに埋め込む手動シミュレーションで行っていたため、この不一致は一度も露見しなかった。`compile.py` がコンパイル時に `.claude-plugin/plugin.json` の `name` を読んで解決する方式で修正（`HARNESS_PLUGIN_ROOT` 未設定時は `harness:` のまま通す fallback あり） |
