...
- **ペルソナレビュー / 任意ゲートの独立コンテキスト実行**: レビュー観点
  ...
  `/gate-run` は diff ベース（レビュー系）だけでなくチケットベース（計画系）の
  ゲートにも使える汎用機構
- **Planner（デフォルト有効）**: Anthropic の harness 設計記事の
  Planner/Generator/Evaluator 構成に倣い、`implement` ワークフローの
  entry ゲートに `plan`（独立コンテキストの `planner` ペルソナ）をデフォルトで
  組み込んだ（v0.6.0）。entry ゲートなので**計画が通るまで実装ファイルの編集自体が
  ブロックされる**。計画は「スコープ / スコープ外 / 影響ファイル / リスク」の
  構成で書かせ、`verify-plan.sh` が空・プレースホルダのまま記録することだけを防ぐ
  （内容の妥当性そのものは自然文のため機械検証できない）
...
├── agents/                  # 汎用ペルソナ（planner / qa / po / architect / security）
...
**エンジン + 全スキル実装済み（v0.9.0）**。E2E スモークテスト 108 件通過（`bash tests/run.sh`）。pokotto-box / classly / ehon-note でパイロット導入済み。**classly では `claude plugin marketplace add` / `claude plugin install`（実インストール、user scope）で通しの動作検証済み**（詳細は下記 v0.9.0）。

**v0.9.0 での修正**: classly への実インストール検証（`claude plugin marketplace add /workspace/claude-harness` → `claude plugin install claude-harness@m-kuwata`）で発覚した、**サブエージェント起動が実運用では機能していなかったバグ**を修正した。
- `personas.<name>.agent` は可読性のため `harness:qa-reviewer` のような短縮形で書く設計だったが、Claude Code がプラグインインストール時に実際に登録する `subagent_type` は `<plugin.json の name>:<名前>`（このプラグインでは `claude-harness:qa-reviewer`）であり、`harness:` という決め打ちの短縮形とは一致しなかった。これまでの検証はすべて手動シミュレーション（`Agent` ツールに `general-purpose` を使いペルソナ定義をプロンプトへ埋め込む代替）で行っていたため、この不一致は一度も露見していなかった
- `scripts/compile.py` に `resolve_agent_prefix()` を追加。コンパイル時に `.claude-plugin/plugin.json` の `name` を読み、`harness:` プレフィックスを実プラグイン名へ解決してから lock に書き込む（`HARNESS_PLUGIN_ROOT` 未設定時は `harness:` のまま通す graceful fallback）。`gate-run-prep.sh` / `review-board-prep.sh` は lock の値をそのまま使うだけなので変更不要
- `scripts/harness-audit.sh` の agent 定義存在チェックが `#harness:` の決め打ち除去をしていたため、解決後の値（`claude-harness:qa-reviewer`）に対して誤検知するようになっていた。名前空間プレフィックスを問わず「最後のコロンまで」を切り落とす方式（`${agent##*:}`）に修正
- 実インストール検証で同時に確認できたこと: user scope でインストールしたプラグインの hooks は、プロジェクトが untrusted（workspace trust dialog 未承認）でも発火する。`.claude/settings.json` の `permissions.allow` はそのプロジェクトの trust に依存して無視されるが、プラグイン側の `PreToolUse` は実際に `Edit` をブロックした（v0.3.0 で見込んでいた「user scope は trust gate の対象外」という設計仮説の実証）

**v0.8.1 での修正**: `harness-audit.sh` が `agent:` 付きゲート（`/gate-run` 経由でサブエージェントとして実行される。`SKILL.md` 不要）に対しても、`agent:` なしゲートと同じ「プロジェクトかプラグインに `SKILL.md` があるか」チェックを一律適用しており、`plan` / `plan-review` のような正しい構成で誤警告（WARN）を出していた。classly への実地適用で発見。`agent:` の有無でチェックを分岐するよう修正

**v0.8.0 での追加**: 「プランナーを再起動して改訂させる仕組み」を自動化した。
- `gate-run-prep.sh` が **reads の逆引き**を行うようになった: あるゲート（例: `plan`）を
  再度 `/gate-run` する際、それを `reads:` しているレビューゲート（例: `plan-review`）が
  却下（`approved:false`）していれば、その feedback/concerns を `revision_from` /
  `revision_feedback` として自動的にバンドルへ含める。Claude が却下理由を手動で
  再構成する必要がなくなり、「計画修正→再レビュー」ループが素直に回せるようになった
  （新しいループ機構は追加していない。既存の verify 再提出ループ + reads を使った）
- `skills/gate-run`: 却下時の手順（5. reads: ベースのゲートが却下された場合）を、
  この自動検出を前提にした簡潔な手順へ書き換えた
- **バグ修正**: jq の `//` 演算子は `false` を偽値として扱いフォールバックしてしまう
  （`false // X` は `X` を返す）。`revision_from` 検出の実装中に、承認判定
  （`approved: false` を明示的に検出したいのに `// empty` の連鎖で握りつぶされる）で
  この罠に再び遭遇し、`!= null` による明示判定に修正した。同様の罠が他にないか
  リポジトリ全体を監査したが、他は真偽の区別が不要な文脈か偶然安全な形だった

**v0.7.0 での追加**: Anthropic 記事の Sprint contracts（実装着手前に Evaluator が
計画を承認する）の完全形として、**plan-review entry ゲートを標準搭載**した。
- `agents/plan-reviewer.md`: プランナーとは別の独立ペルソナ。self-evaluation bias
  （生成者が自分の仕事を甘く評価する傾向。Anthropic 記事が明示的に報告）を避けるため、
  計画を立てたのと同じエージェントには自己レビューさせない
- `gates[].reads`（新フィールド）: 他ゲートの成果物を読んで独立コンテキストの
  サブエージェントに渡す汎用機構。`plan-review` は `reads: plan` で計画本文を取得する
- 却下（`approved:false`）は `verify:` を失敗させ、**同じトークンで再ブロック**
  （既存のノンス・サーキットブレーカー機構をそのまま流用。計画修正→再レビューという
  ループに新しいエンジン機構は追加していない）
- `scripts/verify-approved.sh` を一般化し、review-board の複数ペルソナ形状
  （`all_approved`）と plan-review の単一レビュアー形状（`approved`）の両方に対応

**v0.6.0 での追加**: Anthropic の harness 設計記事（Planner/Generator/Evaluator の
3エージェント構成）を踏まえ、`docs/harness.example.yaml`（デフォルト構成）の
`implement` ワークフローに **Planner を entry ゲートとして標準搭載**した。
- `agents/planner.md`: 独立コンテキストでチケットを「スコープ/スコープ外/
  影響ファイル/実装順序/リスク」に展開する専用ペルソナ。Read/Grep/Glob のみ
  （Write/Edit なし。計画のみで実装はしない）
- entry ゲートなので、**計画が確定するまで実装ファイルの編集自体がブロックされる**
  （Anthropic 記事の「Sprint contracts: 実装前に計画を確定させる」に相当）
- `/gate-run` を diff ベース（レビュー系）だけでなくチケットベース（計画系）にも
  対応するよう一般化し、`gate-run-prep.sh` 1本を plan にも review にも使えるようにした
  （`scripts/plan-prep.sh` のような専用スクリプトは作らず、既存の汎用機構に統合）
- `scripts/verify-plan.sh`: 計画ファイルの実在・最低限の分量を確認する（内容の
  妥当性そのものは自然文のため機械検証できないという限界は review 系と同様）
...
- [x] E2E スモークテスト（tests/run.sh・108 件）
- [x] 実インストール検証（`claude plugin marketplace add` / `install`。classly, user scope）
- [x] パイロット移行: pokotto-box / classly / ehon-note
- [x] ゲート完了の検証強化（gates[].verify・review-board との統合）
- [x] 任意ゲートの独立コンテキスト実行（gates[].agent・/gate-run。diff/チケット/reads 参照の3種対応に一般化）
- [x] セッション/ゲートスコープのアーティファクト（{session_id} 置換・衝突防止）
- [x] 永続進捗ログ（.claude-harness/progress.log・長時間ループの可視化）
- [x] Planner をデフォルト構成の entry ゲートに標準搭載（agents/planner.md・verify-plan.sh）
- [x] plan-review（Sprint contracts の完全形）を標準搭載（gates[].reads・plan-reviewer）
- [x] reads の逆引きによる却下フィードバックの自動注入（プランナー再起動の自動化）
