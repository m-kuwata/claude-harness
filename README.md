...
- **検証可能なアウトプットの強制**（`gates[].verify`）: トークンによる完了申告は
  ...
  通過を受理する（非0なら同じトークンで再ブロック）。review-board はこれを使い、
  ペルソナ（独立コンテキスト）の判定を `.claude-harness/review-findings.json` に
  書き出させ、`scripts/verify-approved.sh` で機械的に確認させる運用を標準とする
...
**エンジン + 全スキル実装済み（v0.4.0）**。E2E スモークテスト 63 件通過（`bash tests/run.sh`）。pokotto-box / classly / ehon-note でパイロット導入済み。
...
review-board はこれを使い、独立コンテキストのペルソナ判定を
`.claude-harness/review-findings.json` に書き出させ `scripts/verify-approved.sh`
で機械的に確認する運用をドキュメント化した。未設定のゲートは引き続き自己申告のまま
（後方互換）。
...
- [x] E2E スモークテスト（tests/run.sh・63 件）
