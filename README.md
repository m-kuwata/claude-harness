...
**エンジン + 全スキル実装済み（v0.6.0）**。E2E スモークテスト 89 件通過（`bash tests/run.sh`）。pokotto-box / classly / ehon-note でパイロット導入済み。
...
- [x] E2E スモークテスト（tests/run.sh・89 件）
- [x] パイロット移行: pokotto-box / classly / ehon-note
- [x] ゲート完了の検証強化（gates[].verify・review-board との統合）
- [x] 任意ゲートの独立コンテキスト実行（gates[].agent・/gate-run。diff/チケット両対応に一般化）
