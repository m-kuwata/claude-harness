#!/usr/bin/env bash
# review-board の準備: 対象 diff を保存し、起動すべきペルソナの定義と
...
[ -z "$sid" ] && { echo "エラー: セッション ID を解決できません" >&2; exit 1; }
...
diff_pending="false"
if [ -n "$pr" ] && [ ! -s "$diff_file" ] && ! command -v gh >/dev/null 2>&1; then
  diff_pending="true"
  echo "⚠ gh がありません。PR diff は GitHub MCP ツールで取得し、$diff_file に保存してから続行してください。" >&2
fi
lines=$(wc -l < "$diff_file" | tr -d ' ')

# ---- アーティファクト出力先（セッション+review-board でスコープ。衝突しない） ----
artifact_path=$(gate_artifact_path "$root" "$sid" "review-board" "findings.json")
